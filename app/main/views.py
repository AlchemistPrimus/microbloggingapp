...
def home():
    ...
    if current_user.can(Permission.WRITE) and form.validate_on_submit():
        #_get_current_object() gets the actual user object required by the database as current_user alone acts as a proxy object
    ...
    #Adding pagination and showing all followed posts
    page = request.args.get('page', 1, type=int)
    #showing all followed posts
    show_followed=False
...
def confirm(token):
    ...
    if current_user.confirm(token):
        db.session.commit()
        flash('Congratulations your account has been confirmed.')
    else:
...
def before_request():
    if current_user.is_authenticated and not current_user.confirmed and request.blueprint !='main' and request.endpoint !='static':
        return redirect(url_for('main.unconfirmed'))

@main.route('/unconfirmed')
def unconfirmed():
    if current_user.is_confirmed or current_user.confirmed:
...
def before_request():
    if current_user.is_authenticated:
        current_user.ping()
        if not current_user.confirmed and request.endpoint and request.blueprint !='main' and request.endpoint != 'static':
            return redirect(url_for('main.unconfirmed'))
...
def followers(username):
    ...
    if user is None:
        ...
        return redirect(url_for('main.home'))
    page = request.args.get('page', 1, type=int)
    pagination = user.followers.paginate(page, per_page=current_app.config['MICROBLOG_FOLLOWERS_PER_PAGE'], error_out=False)
...
def followed_by(username):
    ...
    pagination = user.followed.paginate(
        page, per_page=current_app.config['MICROBLOG_FOLLOWERS_PER_PAGE'],
        error_out=False)
...
#selection of all followed posts
@main.route('/all')
@login_required
def show_all():
