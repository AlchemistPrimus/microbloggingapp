...
class User(UserMixin,db.Model):
    ...
    def __init__(self,**kwargs):
        ...
        super(User,self).__init__(**kwargs)#Invoking constructor base classes
        if self.role is None:
            if self.email==current_app.config['MICROBLOG_ADMIN']:
    ...
    #making users their own followers if we cannot delete and recreate the database we had created to use self.follow(self) option.
    @staticmethod
    ...
    def generate_confiramtion_token(self,expiration=3600):
    ...
    def unfollow(self,user):
        f=self.follow.filter_by(followed_id=user.id).first()
        if f:
            db.session.delete(f)
    ...
    #Role verification(confirming whether a user has a given permission)
    def can(self,perm):
        return self.role is not None and self.role.has_permission(perm)
    def is_administrator(self):
        return self.can(Permission.ADMIN)
    
    #Refreshing user's last visit time
    def ping(self):
        """To keep last visit for user updated, this method should be called each time a user makes a request. It is therefore added before_app_request hook handler."""
    ...
    #avoids duplicating the logic used to compute gravatar_hash
    def gravatar_hash(self):
        return hashlib.md5(self.email.lower().encode('utf-8')).hexdigest()
        
    #Gravatar URL help in showing avatar pictures or the users
    def gravatar(self,size=100,default='identicon',rating='g'):
        if request.is_secure:
            url = 'https://secure.gravatar.com/avatar'
        else:
            url = 'http://www.gravatar.com/avatar'
        #obtaining MD5 hash of the user's email address.The email address is normalized only to contain lowercase letters
        hash = hashlib.md5(self.email.lower().encode('utf-8')).hexdigest()
        return '{url}/{hash}?s={size}&d={default}&r={rating}'.format(url=url, hash=hash, size=size, default=default, rating=rating)
    
    #Obtaining followed posts
    @property
